...
object Main extends Serializable with LazyLogging {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val sparkConfig =
      new SparkConf().set("spark.cores.max", "3").set("spark.cores.min", "3")
    val ssc = new StreamingContext(new SparkContext(sparkConfig),
    ...
    ehSteam.writeToKafka[String, String](
      producerConfig,
      ehData =>
        new ProducerRecord[String, String](topic, new String(ehData.getBody))
    )
    ssc.start()
